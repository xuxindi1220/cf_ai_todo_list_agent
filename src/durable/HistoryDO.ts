  ...
  async fetch(request: Request) {
    ...
    if (request.method === "GET" && pathname === "/histories") {
      try {
        ...
        const listResult: any = await (this.state.storage as any).list?.({ prefix: "session:" })?.catch?.(() => null);

        if (listResult == null) {
          // fallback: try to use keys API if available
          const keys = (await (this.state.storage as any).getKeys?.({ prefix: "session:" })) ?? [];
          ...
          if (typeof (listResult as any)[Symbol.asyncIterator] === "function") {
            for await (const entry of listResult as any) {
              const key = entry?.key ?? (Array.isArray(entry) ? entry[0] : undefined);
              if (!key) continue;
              try {
                const v = await this.state.storage.get(key);
                if (v) items.push(v as StoredSession);
        ...
        return new Response(JSON.stringify({ histories: items }), { status: 200, headers: { "Content-Type": "application/json" } });
      } catch (e: any) {
        return new Response(JSON.stringify({ error: String(e) }), { status: 500, headers: { "Content-Type": "application/json" } });
      }
    }
    ...
    if (request.method === "GET" && pathname.startsWith("/histories/")) {
      ...
      try {
        ...
        if (!v) return new Response(JSON.stringify({ error: "not_found" }), { status: 404, headers: { "Content-Type": "application/json" } });
        return new Response(JSON.stringify({ session: v }), { status: 200, headers: { "Content-Type": "application/json" } });
      } catch (e: any) {
        return new Response(JSON.stringify({ error: String(e) }), { status: 500, headers: { "Content-Type": "application/json" } });
      }
    }
